...
| :---- | :---- |
| **As a new or logged-out user, I want to see a public landing page so that I can understand the app's purpose before signing up.** | \- The default route (`/`) for a logged-out user displays a landing page.\<br\>- The landing page contains the app name, a brief tagline, and an inspirational quote about nature/birdwatching.\<br\>- A prominent "Log In with Google" button is displayed.\<br\>- The main application features (Discovery, My Sightings, etc.) are not accessible until the user is logged in. |
| **As a new or returning user, I want to sign in with my Google account to access the app.** | \- Clicking the "Log In with Google" button initiates the Google Sign-in flow.\<br\>- Upon successful authentication, a new user account is created in Firebase if one doesn't exist.\<br\>- The user is redirected to the main Discovery page after logging in. |
